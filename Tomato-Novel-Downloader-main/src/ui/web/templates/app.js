...
async function refreshJobs() {
  ...
  for (const it of data.items || []) {
    ...
    const deleteHtml = `<button data-jobid="${esc(it.id)}" class="deleteJob sm danger">删除</button>`;
    ...
    tr.innerHTML = `
      ...
      <td><div class="inline-actions">${btnHtml}${deleteHtml}</div></td>
    `;
    tbody.appendChild(tr);
  }
...
function wire() {
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (t.classList.contains('deleteJob')) {
      const id = t.getAttribute('data-jobid');
      if (!confirm(`确认删除 Job ID ${id} 吗？正在运行的任务会被清理出列表。`)) return;
      try { await clearJob(id); } catch (err) { alert(err); }
    }
